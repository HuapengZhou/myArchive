...
namespace ECE141 {
  ...
  struct Testing : public ArchiveObserver {
    ...
    bool doOpenTests(std::ostream& anOutput) {
      ...
      Archive* theArchive = Archive::createArchive(folder + "/test");
      if (theArchive) {
          theArchive->add(folder + "/smallA.txt");
    ...
    bool doAddTests(std::ostream& anOutput) {
      ...
      theArchive = Archive::openArchive(theFullPath);
      if(theArchive) {
    ...
    bool doStressTests(std::ostream& anOutput) {
      ...
      if (Archive* theArchive = Archive::createArchive(thePath)) {
        theArchive->addObserver(*this);
      ...
      if (theResult) {
        ...
        theResult=false;
        if (theArchive && thePreCount && thePreSize) {
